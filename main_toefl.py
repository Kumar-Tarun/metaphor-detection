...
ma = 0
option = 'toefl'
TRAIN_PATH = '/data/toefl/TOEFL_corpus_train.csv'
VAL_PATH = '/data/toefl/TOEFL_corpus_val.csv'
TEST_PATH = '/data/toefl/TOEFL_corpus_test.csv'
...
try:
  with open('/data/toefl/char_vocab.pkl', 'rb') as f:
    char_vocab = pickle.load(f)
except:
  with open('/data/toefl/char_vocab.pkl', 'wb+') as f:
...
glove_embeddings = get_embedding_matrix(word2idx, idx2word, name='toefl', normalization=False)
...
embedded_train_vua = [[embed_indexed_sequence(example[3], example[4], example[0], 
                                              example[2], word2idx, glove_embeddings, 
                                              elmos_train_vua, pos_embeddings),
                       example[2], example[1], get_char_indices(c2idx, example[0])]
                       for example in raw_train_vua]
embedded_val_vua = [[embed_indexed_sequence(example[3], example[4], example[0], 
                                            example[2], word2idx, glove_embeddings, 
                                            elmos_val_vua, pos_embeddings),
                     example[2], example[1], get_char_indices(c2idx, example[0])]
...
print(f'Number of parameters: {rnn_parameters + transformer_parameters}')
...
for epoch in range(num_epochs):
    ...
    for (pos_seqs, example_text, example_lengths, labels, pad_amounts, char_seqs) in train_dataloader_vua:
        ...
        predicted1, embs, p1 = RNNseq_model(example_text, example_lengths, char_seqs)
        predicted2, embs, p2 = Transformer_model(example_text, pad_amounts, char_seqs)
        # combine predictions
        p = (p1 + p2)/2
...
plt.savefig(f'./graphs/toefl/Loss_{im}.png')
...
plt.savefig(f'/graphs/toefl/val_f1_{im}.png')
